...
public class TSBHashtableDA<K,V> implements Map<K,V>, Cloneable, Serializable
{
        ...
        private class KeySetIterator implements Iterator<K>
        {
            ...
            public boolean hasNext()
                {
                    ...
                    if(current == (size() + v)) { return false; }
                    Entry<K,V> entrada;
                    nextCurrent = current + 1;
                    entrada = (Entry<K,V>) table[nextCurrent];
                    for (int i = nextCurrent; i <= (size() + v); i++)
                    {
            ...
            public K next() 
            {
                ...
                if (current == 0){
                    old = current;
                    current = nextCurrent;
                    Entry<K, V> salida = (Entry<K, V>) table[nextCurrent];
                    next_ok = true;
                    return salida.getKey();
                }
                else {
                    old = current;
                    current = nextCurrent;
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> 
    {
        ...
        public boolean contains(Object o) 
        {
            ...
            return TSBHashtableDA.this.contains(o);
        }
